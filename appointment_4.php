<?php
	...
	$patent = mysqli_query($connection,"SELECT pFirstname,pLastname,pPhoneNo,pEmail,pCmDisease FROM `patientdata` WHERE pId ='$sid'");
	$data_p = mysqli_fetch_array($patent);

	$doctor = mysqli_query($connection,"SELECT dName,dPhoneNo,dEmail FROM `doctordata` WHERE dId ='$docid'");
	$data_d =mysqli_fetch_array($doctor);

	$q3= mysqli_query($connection,"SELECT * FROM `doctime` where dId = '$docid' AND day= '$f_day' AND timee = '$times'");
